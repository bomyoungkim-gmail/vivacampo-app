...
## Status
- **Severity P0 – RBAC/tenant isolation enforcement (now covered):** `tests/test_rbac.py` adds `VIEWER` vs. `OPERATOR` enforcement plus a cross-tenant backfill attempt. That file now exercises the negative paths that were previously missing, so the multi-tenant security boundary is backstopped by tests.
